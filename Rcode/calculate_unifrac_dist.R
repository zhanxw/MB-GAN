...
load("phylo_tree_info.Rdata", verbose = T)

#### 1. case group ####
# (a) load the real data #
load("Data_NielsenHB_real.Rdata", verbose = T)
load("Data_by_WGAN_v1.Rdata", verbose = T)
...
#### 2. control group ####
setwd("~/archive/MB-GAN/revision_1/nmds_analysis/Data/")
# (a) load the real data #
load("Data_NielsenHB_real.Rdata", verbose = T)
load("Data_by_WGAN_v1.Rdata", verbose = T)
load("Data_by_NorTA.Rdata", verbose = T)
...
save.nam = paste0("../daat/unifrac_nmds.Rdata")
